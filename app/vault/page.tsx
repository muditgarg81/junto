...
import { Folder, ArrowRight, Calendar, MapPin, Sparkles } from 'lucide-react';

export default async function GeneralVaultPage() {
  // Fetch all trips alongside their vault item count
  const tripsRes = await query(`
    SELECT t.*, COUNT(v.id) as vault_count
    FROM trips t
    LEFT JOIN vault_items v ON t.id = v.trip_id
    GROUP BY t.id
  ...
  return (
    <div className="relative min-h-screen bg-surface flex flex-col justify-between max-w-md mx-auto w-full border-x border-border-warm-grey shadow-sm">
      <div className="flex-grow px-6 py-8 space-y-8 pb-24 z-10">
        ...
        {/* Header */}
        <div>
          <h1 className="font-display text-4xl text-ink-text leading-tight font-bold">Document Vault</h1>
          <p className="font-body-sm text-muted-text">Select a trip to access or upload travel vouchers</p>
        </div>
        ...
        <div className="space-y-4">
          {trips.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-12 space-y-3 opacity-60">
              <Folder className="w-12 h-12 text-[#1f4d3f]" />
            ...
            <div className="space-y-3">
              {trips.map((trip) => {
                const count = parseInt(trip.vault_count, 10);
                return (
                  <Link
                    key={trip.id}
                    href={`/trip/${trip.id}/vault`}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-text font-body-sm">
                          <span className="flex items-center gap-1">
                            <Folder className="w-3.5 h-3.5 text-[#1f4d3f]" />
      ...
      <footer className="bg-card-cream border-t border-border-warm-grey sticky bottom-0 left-0 right-0 z-30 max-w-md mx-auto w-full px-6 py-4 flex justify-between items-center shadow-md">
        ...
        <span className="font-label-caps text-[10px] text-muted-text">Junto Document Vault</span>
      </footer>
    </div>
  );
}
